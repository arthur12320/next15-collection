...
import type { SelectCollectionWithUser } from "@/db/schema/collections";
...
export default function CollectionCard({
  ...
  const cardContent = (
    <Card
      className={`w-full transition-all duration-300 hover:shadow-lg ${collection.id === "loading" ? "opacity-50" : "cursor-pointer"}`}
    >
      <CardHeader>
        <div className="flex w-full items-center justify-between">
          <CardTitle className="text-2xl text-center font-bold">{collection.name}</CardTitle>
          <Button
            variant="destructive"
            size="icon"
            ...
            disabled={isDeleting || collection.id === "loading"}
          >
            {isDeleting ? (
      ...
      <CardContent>
        <div className="flex w-full items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">{collection.user.name}</p>
          </div>
        </div>
      </CardContent>
